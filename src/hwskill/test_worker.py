...
def _parse_request(value: Any) -> WorkerRequest:
    ...
    _exact_keys(data, {
        "schema_version", "selections", "host", "host_version", "model", "reasoning",
    ...
    try:
        ...
        host_version = _string(data["host_version"], "host_version")
        if host_version != HOST_SPECS[host].verified_version:
            raise ValueError("host_version does not match the standard image contract")
...
def execute_worker(
    ...
    *,
    ...
    """Execute a validated request within fixed execution roots and an optional host export root."""
    ...
    try:
        ...
        exported = _validate_export_root(export_root, repository, tests, artifacts, workspace)
        ...
        environment = TestEnvironment(
            ...
            agent_command_prefix=(
                ...
                "--read-only", str(repository), "--read-only", str(tests),
                "--read-only", str(artifacts), "--read-only", "/credentials",
                "--read-write", str(workspace), "--",
            ),
        )
        ...
        code = {"PASS": 0, "FAIL": 1, "BLOCKED": 3}[status]
        return _finish_worker(result, artifacts, exported, environment.secret_values, code)
        ...
        try:
            ...
            target = exported or artifacts
            if not (target / "result.json").exists():
                _write_worker_result(result, target, request.selections)
...
def _validate_export_root(
    ...
    if export_root is None:
        return None
    exported = _real_mount_root(export_root, "artifact export root", writable=True)
    for other in (repository, tests, artifacts, workspace):
        other = Path(other).absolute()
        if exported == other or exported.is_relative_to(other) or other.is_relative_to(exported):
            raise WorkerRequestError("artifact export root must be separate from every execution mount")
    if os.listdir(exported):
        raise WorkerRequestError("artifact export root must be empty")
    return exported
...
def _finish_worker(
    ...
    if export_root is None:
        return code
    unsafe = _export_artifacts(staging_root, export_root, secrets, exclude=("result.json",))
    unsafe = _export_artifact_result(staging_root, export_root, secrets) or unsafe
    if not unsafe:
        return code
    export_fd = os.open(export_root, os.O_RDONLY | os.O_DIRECTORY | getattr(os, "O_NOFOLLOW", 0))
    try:
        try:
            os.unlink("result.json", dir_fd=export_fd)
        except FileNotFoundError:
            pass
    finally:
        os.close(export_fd)
    blocked = TestRunResult(
        "BLOCKED", export_root, (), "docker", result.host, result.model, result.host_version,
        blocked_reason="test artifact export rejected unsafe credential-bearing content",
    )
    _write_worker_result(blocked, export_root)
    return 3


def _export_artifact_result(staging_root: Path, export_root: Path, secrets: tuple[str, ...]) -> bool:
    source_fd = os.open(staging_root, os.O_RDONLY | os.O_DIRECTORY | getattr(os, "O_NOFOLLOW", 0))
    destination_fd = os.open(export_root, os.O_RDONLY | os.O_DIRECTORY | getattr(os, "O_NOFOLLOW", 0))
    try:
        return _export_regular_file(source_fd, destination_fd, "result.json", _secret_tokens(secrets), [0, 0])
    finally:
        os.close(destination_fd)
        os.close(source_fd)


def _secret_tokens(secrets: tuple[str, ...]) -> tuple[bytes, ...]:
    tokens = tuple(dict.fromkeys(value.encode("utf-8") for value in secrets if value))
    if any(len(token) > _MAX_SECRET_BYTES for token in tokens):
        raise WorkerRequestError("configured credential exceeds artifact inspection limit")
...
def _export_artifacts(
    ...
    *,
    ...
    """Export only no-follow regular artifacts while removing configured raw secrets."""
    source = _real_mount_root(staging_root, "artifact staging root", writable=True)
    destination = _real_mount_root(export_root, "artifact export root", writable=True)
    if source == destination or source.is_relative_to(destination) or destination.is_relative_to(source):
        raise WorkerRequestError("artifact staging and export roots must be separate")
    if os.listdir(destination):
        raise WorkerRequestError("artifact export root must be empty")
    source_fd = os.open(source, os.O_RDONLY | os.O_DIRECTORY | getattr(os, "O_NOFOLLOW", 0))
    destination_fd = os.open(destination, os.O_RDONLY | os.O_DIRECTORY | getattr(os, "O_NOFOLLOW", 0))
    state = [0, 0]
    try:
        return _export_directory(source_fd, destination_fd, (), _secret_tokens(secrets), state, frozenset(exclude))
    finally:
        os.close(destination_fd)
        os.close(source_fd)
...
def _export_directory(
    ...
    if len(relative) > _MAX_ARTIFACT_DEPTH:
        return True
    unsafe = False
    for name in sorted(os.listdir(source_fd)):
        joined = "/".join((*relative, name))
        if not relative and name in exclude:
            continue
        if _unsafe_artifact_name(joined, tokens):
            unsafe = True
            continue
        try:
        ...
        if stat.S_ISDIR(metadata.st_mode):
            ...
            try:
                child_source = os.open(
                    name, os.O_RDONLY | os.O_DIRECTORY | getattr(os, "O_NOFOLLOW", 0), dir_fd=source_fd,
                )
                os.mkdir(name, 0o700, dir_fd=destination_fd)
                child_destination = os.open(
                    name, os.O_RDONLY | os.O_DIRECTORY | getattr(os, "O_NOFOLLOW", 0), dir_fd=destination_fd,
                )
            except OSError:
                ...
                continue
            try:
                unsafe = _export_directory(
                    child_source, child_destination, (*relative, name), tokens, state, exclude,
                ) or unsafe
            finally:
                os.close(child_destination)
                os.close(child_source)
            ...
            state[0] += 1
            if state[0] > _MAX_ARTIFACT_FILES:
                unsafe = True
                continue
            unsafe = _export_regular_file(source_fd, destination_fd, name, tokens, state) or unsafe
        else:
            unsafe = True
    return unsafe


def _unsafe_artifact_name(relative: str, tokens: tuple[bytes, ...]) -> bool:
    encoded = os.fsencode(relative)
    return (
        any(byte < 32 or byte == 127 for byte in encoded)
        or any(token in encoded for token in tokens)
    )
...
def _export_regular_file(
    ...
    try:
        ...
        if not stat.S_ISREG(before.st_mode) or before.st_nlink != 1:
            return True
        if before.st_size > _MAX_ARTIFACT_FILE_BYTES or state[1] + before.st_size > _MAX_ARTIFACT_TOTAL_BYTES:
            return True
        state[1] += before.st_size
        destination = os.open(
            name, os.O_WRONLY | os.O_CREAT | os.O_EXCL | getattr(os, "O_NOFOLLOW", 0), 0o600,
            dir_fd=destination_fd,
        )
        redacted = _copy_redacted(source, destination, tokens)
...
def _copy_redacted(source_fd: int, destination_fd: int, tokens: tuple[bytes, ...]) -> bool:
    pending = b""
    redacted = False
    maximum = max((len(token) for token in tokens), default=1)
    while True:
        block = os.read(source_fd, 64 * 1024)
        eof = not block
        pending += block
        while pending:
            safe_limit = len(pending) if eof else max(0, len(pending) - maximum + 1)
            matches = [
                (position, -len(token), token)
                for token in tokens
                if (position := pending.find(token)) >= 0 and (eof or position < safe_limit)
            ]
            if not matches:
                if safe_limit:
                    _write_all(destination_fd, pending[:safe_limit])
                    pending = pending[safe_limit:]
                break
            position, _negative_length, token = min(matches)
            _write_all(destination_fd, pending[:position])
            _write_all(destination_fd, _REDACTED_BYTES)
            pending = pending[position + len(token):]
            redacted = True
        if eof:
            break
    return redacted
...
def _write_all(descriptor: int, value: bytes) -> None:
    ...
    while remaining:
        ...
        if written <= 0:
            raise OSError("short artifact export write")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        return 3
    return execute_worker(request, export_root=Path("/export"))
