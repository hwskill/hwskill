...
def _validate_catalog_documents(snapshot: FeedSnapshot) -> None:
    ...
    for url in sorted(urls):
        ...
        errors = sorted(validator_for("catalog").iter_errors(document), key=lambda error: list(error.absolute_path))
        if errors:
...
def validate_snapshot(snapshot: FeedSnapshot, *, validate_subjects: bool = True) -> FeedSnapshot:
