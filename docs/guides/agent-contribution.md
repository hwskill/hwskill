...
3. 记录来源链接、查询日期、许可证、兼容性和限制。入口技能依赖其他技能时，在 `install.included_skills` 列出完整安装集合，并在 requirements 或 limitations 说明依赖关系。不能确认的值写 `unknown`，安装/行为未运行必须明确为 `not_run` 或 unknown。
