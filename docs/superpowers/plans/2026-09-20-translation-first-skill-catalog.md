...
|---|---|---|---|---|
| H1 | License `local/gitcode-discussion-fetch` and `local/gitcode-pr-review-fetch`, including their translations, under MIT so redistribution and modification are allowed | Repository owner | Supplied on 2026-09-20; record in Task 5 | Root `LICENSE` plus `license.status: known`, `identifier: MIT`, and repository license URL in both Entries |
...
Agent work that can be prepared immediately: record the supplied H1 license decision, generate the source-ref ledger, draft all translations, verify file/link/code-block coverage, implement schemas and UI, and prepare the branch-protection runbook. H2-H3 remain human judgments; absence of translation approval keeps affected Entries in draft.

## Delivery Order

1. Complete Tasks 1-4 in this repository.
2. Draft Tasks 5-7 while humans review H2-H3; implement the supplied H1 MIT decision in Task 5.
...
- [ ] **Step 2: Record the supplied H1 decision.** Add the MIT license text at repository root. Set both hosted Entries to `license.status: known`, `identifier: MIT`, and `url: https://github.com/hwskill/hwskill/blob/main/LICENSE`. This authorization covers source redistribution, modification, and Chinese translations.
...
- [ ] **Step 1: Add final build assertions.** Require 44 publishable Entries, one translation per publishable Entry, all original links, four recommendations, and no removed fields in current generated artifacts.
...
- Human items H1-H6 have evidence; H1 is implemented as MIT and unresolved translation review keeps only the affected skill in draft.
