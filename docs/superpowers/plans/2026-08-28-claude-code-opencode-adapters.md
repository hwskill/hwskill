...
- [x] **Step 2: 运行 `PYTHONPATH=src python3 -m unittest tests.test_host_adapters -v`，确认因模块不存在而失败**
- [x] **Step 3: 实现最小 renderer 和 wrapper；Codex 改为调用 renderer，不改变现有 JSON 契约**
- [x] **Step 4: 重跑目标测试和 `tests.test_codex_runtime`，确认全部通过**
- [x] **Step 5: 提交 `feat: share effective catalog across host adapters`**
...
- [x] **Step 1: 写失败测试，覆盖空项目、保留旧配置、幂等、冲突和外部修改拒绝卸载**
...
- [x] **Step 2: 运行 `PYTHONPATH=src python3 -m unittest tests.test_host_configuration -v`，确认缺少接口而失败**
- [x] **Step 3: 实现 JSON 读写、规范化摘要、冲突检测、精确删除和受管 OpenCode 插件模板**
- [x] **Step 4: 重跑配置测试；用 `node --check <临时插件路径>` 验证生成 JavaScript 语法**
- [x] **Step 5: 提交 `feat: configure Claude Code and OpenCode projects`**
...
- [x] **Step 1: 写失败 CLI 测试，验证 `claude_code` 别名、两种 setup/unsetup、adapter 输出和 doctor JSON**
- [x] **Step 2: 写失败 doctor 测试，以 patch 的 `shutil.which/subprocess.run` 固定 OpenCode 1.14.48 为 PASS、其他版本为 WARN**
- [x] **Step 3: 运行两份目标测试，确认 parser choices/旧签名导致预期失败**
- [x] **Step 4: 增加 host 归一化和路由；把 doctor 拆为共享检查 + Codex/Claude/OpenCode 检查，保持 `doctor codex` 兼容**
- [x] **Step 5: 运行目标测试与完整单元测试，确认无回归**
- [x] **Step 6: 提交 `feat: expose Claude Code and OpenCode host commands`**
...
- [x] **Step 1: 用脱敏的 Claude stream-json 与 OpenCode JSON fixture 写失败测试，分别产生公共 search/load/command 事件**
- [x] **Step 2: 增加失败测试：echo 提及路径不算执行、Load 前执行不算直达、Load 后 find 被报告为发现行为**
- [x] **Step 3: 运行两份 observer 测试，确认因 normalizer/host 参数缺失而失败**
- [x] **Step 4: 实现宿主 normalizer；将当前 Codex 解析保留为默认路径，公共判定逻辑不放宽 argv 匹配**
- [x] **Step 5: 运行 observer 测试和完整单元测试**
- [x] **Step 6: 提交 `feat: observe skill resolution across agent hosts`**
...
- [x] **Step 1: 先扩展离线 entrypoint，使其 setup/adapter/doctor 两种宿主；运行 `bash scripts/run_docker_smoke.sh` 确认镜像尚缺所需 Agent 或命令而失败**
- [x] **Step 2: 固定安装 Claude Code 2.1.141 与 OpenCode 1.14.48，加入容器内凭据读取但不输出 token 的启动逻辑**
- [x] **Step 3: 实现两个宿主评测脚本，使用相同无路径提示词，输出原始事件并调用公共 observer 生成报告**
- [x] **Step 4: 更新 README，记录 setup 命令、版本边界、凭据来源、运行命令和直达判定语义**
- [x] **Step 5: 运行离线全套：`PYTHONPATH=src python3 -m unittest discover -s tests -v`、registry validate/build check、Docker smoke**
- [x] **Step 6: 运行 Claude Code 真实评测，核对 patch、audit 和报告中的 `direct_resolution/execution_succeeded`**
- [x] **Step 7: 运行 OpenCode 真实评测，核对相同字段；外部失败时保留诊断且不得生成通过报告**
- [x] **Step 8: 扫描仓库与 artifacts，确认没有 MiniMax token、受管原生 skill 目录或意外凭据文件**
- [x] **Step 9: 提交 `test: cover Claude Code and OpenCode agent execution`**
...
- [x] **Step 1: 使用 `superpowers:verification-before-completion` 重跑全部离线验证并记录准确输出**
- [x] **Step 2: 使用 `superpowers:requesting-code-review` 检查规范覆盖、配置安全、凭据泄漏和观察器假阳性**
- [x] **Step 3: 对审查问题先写失败测试再修复，重跑相关测试与全套测试**
- [x] **Step 4: 检查 `git diff --check`、`git status --short` 和提交历史，确保仅包含本轮范围内文件**
