...
if [ ! -r "$auth_source" ]; then
  echo "OpenCode GitCode eval requires the existing MiniMax login: $auth_source" >&2
  exit 2
fi

credential_dir=$(mktemp -d /tmp/hwskill-opencode-minimax-XXXXXX)
...
if HWSKILL_MINIMAX_AUTH_FILE="$filtered_auth" "$hwskill" \
  ...
  --runner docker --host opencode --repo-root "$repo_root" "$@"; then
  exit 0
else
  exit $?
fi
