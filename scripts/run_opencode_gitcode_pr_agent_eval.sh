...
run_root=$(mktemp -d /tmp/hwskill-opencode-agent-XXXXXX)
...
artifact_parent="$repo_root/artifacts/opencode-gitcode-pr-agent-eval"
...
if docker run --rm \
  ...
  --entrypoint /opt/hwskills/docker/opencode-gitcode-agent-eval.sh \
  "$image_name"; then
  status=0
else
  status=$?
